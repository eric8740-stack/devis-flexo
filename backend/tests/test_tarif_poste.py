...
def test_seed_loads_10_tarifs_poste():
    ...
    with SessionLocal() as db:
        assert len(list_tarifs_poste(db, entreprise_id=1)) == 10


def test_get_by_cle_returns_matching_record():
    with SessionLocal() as db:
        tarif = get_by_cle(db, "matiere_prix_kg_defaut", entreprise_id=1)
...
def test_get_by_cle_returns_none_when_missing():
    with SessionLocal() as db:
        assert get_by_cle(db, "cle_inexistante_xyz", entreprise_id=1) is None
...
def test_create_tarif_poste_persists():
    ...
    with SessionLocal() as db:
        created = create_tarif_poste(db, payload, entreprise_id=1)
        assert created.id is not None
        assert created.actif is True
        # vérifie qu'il est retrouvable par cle
        assert get_by_cle(db, "test_param_unique", entreprise_id=1).id == created.id


def test_update_tarif_poste_modifies_field():
    # Le seed contient cliche_prix_couleur (id=2, valeur 45)
    with SessionLocal() as db:
        original = get_by_cle(db, "cliche_prix_couleur", entreprise_id=1)
