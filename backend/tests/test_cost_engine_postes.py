...
def test_p2_encres_sum_per_type():
    ...
    """
    with SessionLocal() as db:
        result = CalculateurPoste2Encres(db, entreprise_id=1).calculer(_devis_median())
...
def test_p2_encres_raises_on_unknown_type_encre():
    ...
    with SessionLocal() as db:
        with pytest.raises(CostEngineError, match="type_encre"):
            CalculateurPoste2Encres(db, entreprise_id=1).calculer(devis)
...
def test_p3_v1a_existing_tool_5_couleurs():
    ...
    with SessionLocal() as db:
        result = CalculateurPoste3ClichesOutillage(db, entreprise_id=1).calculer(_devis_median())
...
def test_p4_calage_forfait():
    ...
    with SessionLocal() as db:
        result = CalculateurPoste4Calage(db, entreprise_id=1).calculer(_devis_median())
    assert result.montant_eur == Decimal("225.00")
...
def test_p5_roulage_mark_andy_3000ml():
    ...
    with SessionLocal() as db:
        result = CalculateurPoste5Roulage(db, entreprise_id=1).calculer(_devis_median())
...
def test_p5_roulage_raises_on_unknown_machine():
    ...
    with SessionLocal() as db:
        with pytest.raises(CostEngineError, match="Machine"):
            CalculateurPoste5Roulage(db, entreprise_id=1).calculer(devis)
...
def test_p6_finitions_base_plus_st():
    ...
    with SessionLocal() as db:
        result = CalculateurPoste6Finitions(db, entreprise_id=1).calculer(_devis_median())
...
def test_p6_finitions_without_st_only_base():
    ...
    with SessionLocal() as db:
        result = CalculateurPoste6Finitions(db, entreprise_id=1).calculer(devis)
...
def test_p7_mo_derived_from_machine():
    ...
    with SessionLocal() as db:
        result = CalculateurPoste7MO(db, entreprise_id=1).calculer(_devis_median())
...
def test_p7_mo_uses_override_when_provided():
    ...
    with SessionLocal() as db:
        result = CalculateurPoste7MO(db, entreprise_id=1).calculer(devis)
