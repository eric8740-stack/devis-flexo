...
export default function EditDevisPage() {
  ...
  // Redirection auto vers /optimisation?devis_id=X pour les devis multi-lots.
  // Le panneau complet (saisie + candidats + détail lots + chiffrage) y est
  // hydraté par `hydrateFromDevisExistant` qui bascule sur l'étape
  // "chiffrage" par défaut. Cette page reste l'éditeur unique pour les
  // devis mono-lot legacy via DevisCalculForm.
  const isMultiLots = devis
    ? (devis.lots_production?.length ?? 0) > 0 ||
      (devis.payload_output as Record<string, unknown>)?.mode === "multi-lots"
    : false;

  useEffect(() => {
    if (devis && isMultiLots) {
      router.replace(`/optimisation?devis_id=${devis.id}`);
    }
  }, [devis, isMultiLots, router]);
  ...
  // Frame intermédiaire pendant le `router.replace` : on évite de monter
  // brièvement DevisCalculForm avant la redirection effective.
  if (isMultiLots) {
    return (
      <main className="container mx-auto max-w-5xl p-4 sm:p-8">
        <div className="text-sm text-muted-foreground">
          Redirection vers l&apos;optimisation…
        </div>
      </main>
    );
  }
