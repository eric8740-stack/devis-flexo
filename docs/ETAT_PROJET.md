...
- **Date** : 2026-07-06
- **Branche active** : `main` (après #157 blindage · #158 · #159 D1 · #160 D2 · #161 Lot 4b · **#162 F2 back + PR F2 front**). **LOT D et LOT F2 COMPLETS EN PROD end-to-end.**
...
- **Carte qui-fait-quoi** : **CC1 / CC2 = libres.** ~~Lot 3 renommages P5/P7~~ **FAIT (#72** — P5/P7 lisent `ConfigCouts.cout_exploitation_machine_eur_h` / `cout_operateur_eur_h` scopés tenant**)** · ~~Lot 4a (7 champs backend)~~ **FAIT** · ~~Lot 4b (UI Stratégique)~~ **FAIT (cette PR)**. ~~extension planificateur bobines~~ **Lot F2 FAIT (#162 back + cette PR front — imposé + surplus au `/preview` ; le planificateur l'avait depuis #73)**. **Prochain : épaisseur paroi mandrin · cleanup TarifPoste (suppression des colonnes dépréciées — lot dédié). **Restes d'audit (non urgents)** : M1 rate limiting auth · M2 rotation refresh + cookie HttpOnly · idempotence création devis (double-clic = 2 devis) · M3 `correspondance_laize_metrage` sans `entreprise_id` · M4 fallback silencieux laize papier · Next 15/16 · client API généré OpenAPI · lock requirements. Lot « facturation temps d'arrêt » = lot DÉDIÉ ultérieur (touche cost_engine → re-baseline).

---

## 2026-07-06 · Lot F2 — nb de bobines imposé au /preview + surplus n_laize (#162 back + PR front)

- **Back (#162, mergé, vérifié en prod)** : `DevisPreviewIn.nb_bobines_impose` (ge=1), **value-neutral strict** (sans imposé, clés F2 ABSENTES de la réponse via model_serializer — bloc bobinage octet pour octet identique au Lot F). Imposé → `ml_par_bobine` dérivé, Ø recalculé (bat_calculs lecture pure), `depasse_max` conservée ; fourni avec `ml_par_bobine` → l'imposé prime + alerte info (pas de 422, esprit Règle 7). **Surplus** : paquets de `n_laize` (même source que le planificateur #73 — poses en laize post-optim, `nb_filles` en sans-outil) → `nb_bobines_production` + `surplus_bobines` + alerte. cost_engine diff vide, 0 migration, sacrés confirmés. OpenAPI prod vérifié AVANT le front (leçon #137).
- **Front (cette PR)** : toggle « Imposer le nombre de bobines » (bloc Bobinage & livraison, pattern checkbox sans-outil) — payload EXCLUSIF garanti dans `buildPreviewRequest` (clé non active omise, jamais les deux). Bandeau surplus info « N demandées → M produites, +X » + formulation backend, « (M produites) » au plan bobines, dégradation propre si clés F2 absentes (back ancien). +7 tests (devisPreview 4, page 3) — vitest 271/271.

---

## 2026-07-05 · Lot 4b — UI Stratégique : les 7 coûts du Lot 4a éditables (front pur)

- **Pré-vol leçon #137 validé** : OpenAPI prod (`devis-flexo-production.up.railway.app`) — les 7 champs présents sur `ConfigCoutsRead` ET `ConfigCoutsUpdate` (`GET/PUT /api/strategique/couts`).
- **Livré** : nouveau `ConfigCoutsChamps.tsx` (GET singleton + grille label/unité + **PUT partiel strict** : seuls les champs modifiés partent, clic sans modif = zéro appel) décliné en 4 sections — **Outils** (cliché €/couleur, outil base €, outil €/tracé, surcoût forme spéciale ×1–10 avec aide inline) · **Calage** (forfait €) · **Finitions** (€/m²) · **Roulage** (marge confort mm, sous-groupe de la section existante). Sections nav Calage/Finitions ajoutées.
- **`OutilsSection` readonly TarifPoste SUPPRIMÉE** (UI mensongère : clés dépréciées que le moteur n'utilise plus depuis le Lot 4a). Colonnes TarifPoste elles-mêmes intactes → lot « cleanup TarifPoste » séparé. Machines/Complexes/Encre readonly conservées (encore utilisées).
- **Front pur** : zéro fichier backend, zéro migration, seeds/défauts intacts, sacrés hors de portée (backend 1287 inchangé). Backlog : « Lot 3 renommages » retiré (fait par #72).
- Validations : tsc 0 · lint 0 · **vitest 264/264** (+8 tests : rendu GET, PUT partiel `toEqual`, no-op sans modif, erreurs GET/PUT) · build OK.
