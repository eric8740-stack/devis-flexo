...
- **Date** : 2026-06-05 (fin de journée)
- **Branche active** : `main` = **`7708857`** (après #117 — spec « format sans outil » consignée). **Lot back A NON mergé** (PR #118 ouverte, à merger demain).
- **Sprint en cours** : **Chantier « format sans outil »** (cf. section dédiée). **Lot back A livré** (PR #118, vert local + à valider/merger demain). Chantiers CLOS mergés+déployés : **L1** (géométrie laize), **L2** (rebasage P1 + plafond, sacrés re-baselinés), **Souveraineté** (forçages Règle 7 non bloquants). Bugs #5/#6 **CLOS**.
- **Carte qui-fait-quoi** : **CC1** = lot back A (PR #118, en attente merge demain). **CC2** = stand-by (lot front « sans outil » à venir, après merge back A).
...
- **pytest local — `main`** (SQLite, `PG_TEST_URL` absent) : `1148 passed, 10 skipped, 0 failed`. Skips : 2 tests subprocess SQLite migration P1+P2, 2 tests obsolètes `ConfigurationPose` / `MachineImprimerie`-spec (tables droppées), 1 test PG sous FK strictes (skip si `PG_TEST_URL` absent → tourne en CI uniquement), 5 autres skip historiques env-dependent.
- **pytest local — branche lot back A (PR #118, NON mergée)** : `1162 passed, 10 skipped, 0 failed` (**+14** tests « format sans outil » : géométrie pure, validateur, endpoint sans/avec outil, pont cost). Sacrés L2 EXACTS (flag `mode_sans_outil=False` par défaut → value-neutral).
...
- **alembic** : HEAD `main` = **`f7a8b9c0d1e2`** (L1 géométrie laize, #107 ; L2 #114 n'ajoute aucune migration). Lot back A (**PR #118, non mergée**) ajoute **`a2b4c6d8e0f1`** (`lot_production.mode_sans_outil` + `laize_stock_mm` + `cylindre_id` nullable — additive, réversible, dialect-aware) → deviendra le head au merge. Précédents `e6f7a8b9c0d1` (type_machine) → `d5e6f7a8b9c0` (paroi mandrin) → `c4d5e6f7a8b9` (config_couts ICE) → `b2c3d4e5f6g7` (P1+P2 unify). Application prod auto via `CMD` Dockerfile.
...
- **▶ Reprise demain (06/06)** : **POINT PROJET** → **vérif `nb_filles`** (exposé via `nb_poses_laize` ; décider de l'expliciter dans `geometrie_laize`) → **merge PR #118** (lot back A) → enchaîner **lot back B** (refente additive via module rebobinage) + **lot front** (toggle Card Format + masquages + ligne déchet latéral, CC2).
...
> **Lot back A livré** (PR #118, à merger demain). Cartographie back + décisions
> + découpage en fin de section.
...
- **Lot back A — ✅ LIVRÉ (PR #118, `a79804b`, à MERGER DEMAIN).** Vert local + CI **1162 passed / 10 skipped / 0 failed** (1148 + 14 tests sans outil). Définit le **contrat /calculer** que le front consommera.
  - **Contrat** : `DevisInput.mode_sans_outil` + `laize_stock_mm` ; `OptimisationCalculerRequest.mode_sans_outil` + `laize_stock_mm` (obligatoire si sans outil, validateur). `LotProduction.mode_sans_outil` + `laize_stock_mm` + **`cylindre_id` NULLABLE** (sentinelle `0` côté config), migration **`a2b4c6d8e0f1`** (additive, réversible, dialect-aware).
  - **Sortie** : `geometrie_laize` **+3 clés** (`laize_stock_mm` / `laize_utile_mm` / `dechet_lateral_mm`, `None` en mode avec outil) + écho top-level `mode_sans_outil`. Court-circuit candidats cylindre (`optimiser_pose` bypassé), `intervalle_dev=0`, géométrie pure [`sans_outil.py`](../backend/app/services/optimisation/sans_outil.py).
  - **Décisions appliquées** : Option A (champ explicite `laize_stock_mm`) ; intervalle refente défaut **3 mm** (`DEFAULT_INTERVALLE_REFENTE_MM`, acté Eric) ; **bord_lateral L1 NON appliqué** en sans outil. P1 facture la laize STOCK entière (déchet inclus). **cost_engine logique INTOUCHÉE** ; `rotation_se` / `bat_calculs` **INTOUCHÉS**.
  - **⚠️ Point ouvert AVANT merge (demain)** : `nb_filles` est actuellement exposé via `config.nb_poses_laize` (égal au nb de filles en sans outil), **pas** comme champ explicite dans `geometrie_laize`. Décision demain : l'expliciter (`geometrie_laize.nb_filles`) ou conserver `nb_poses_laize`.
