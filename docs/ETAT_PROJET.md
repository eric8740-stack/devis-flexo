...
- **pytest local** (SQLite, `PG_TEST_URL` absent) : `1136 passed, 10 skipped, 0 failed` — main + bug #5 calage 1 par montage (+5 tests : dédup agrégateur unitaire + tripwire e2e « 2 lots même montage »). Skips : 2 tests subprocess SQLite migration P1+P2 (limitation FK enforcement vs alembic transaction, documentée), 2 tests obsolètes `ConfigurationPose` / `MachineImprimerie`-spec (tables droppées), 1 test PG sous FK strictes (skip si `PG_TEST_URL` absent → tourne en CI uniquement), 5 autres skip historiques env-dependent.
- **pytest CI** (service `postgres:16`) : `1142 passed, 4 skipped, 0 failed` — inclut `test_migration_p1p2_sous_fk_strictes_postgres` qui valide la migration P1+P2 sous **FK strictes Postgres** (scénario réel boot Railway prod). Seuls les 4 skips inévitables restent (2 SQLite subprocess + 2 modèles obsolètes).
...
- **Bug #5 — 1 calage par montage (dédup du calage des lots de même signature)** — branche `feat/bug5-calage-par-montage`. **BACKEND only**. Le calage (P4) est lié à l'**outil** (plaque+clichés), pas à la bobine : un devis multi-lots comptait **N calages** (225 € × N) alors qu'un seul montage = 1 calage. Fix **EN AVAL, hors moteur** (mono-config V1a INTACT) : `cost_engine_aggregator.calculer_devis_multilots` accepte des **`montage_signatures`** par lot = `(cylindre_id, machine_id, nb_poses_dev, nb_poses_laize)` ; les lots **2+** d'une signature déjà vue voient leur **poste 4 déduit** (`cout_revient − calage`, prix recalculé `× (1+marge)`). `_chiffrer_devis_multilots` (crud) construit les signatures depuis `LotProduction`. **Heuristique signature** (zéro migration/UI ; l'override « changement d'outil/cliché » reste backlog CC2). Trace d'audit `details["calage_montage_deduplique_eur"]` par lot. **SACRED INCHANGÉ** : un devis à **1 lot** n'est JAMAIS touché (1ʳᵉ signature conservée) → **V1a 1 449,09 € + tripwire P0b 704,07 € EXACT**. `montage_signatures=None` → comportement legacy (somme pure). +5 tests (dédup unitaire agrégateur + tripwire e2e non-sacré « 2 lots même montage »). ZÉRO front, `poste_4_calage`/`bat_calculs`/`rotation_se` intouchés. Baseline **1136**.
